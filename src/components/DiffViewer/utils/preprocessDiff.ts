...
import type { CollapsedLine, DiffRow, DiffRowOrCollapsed, SegmentItem } from "../types";

const DEFAULT_CONTEXT_SIZE = 3;
const DEFAULT_COLLAPSE_THRESHOLD = 10;

function shouldAddTopContext(index: number, segments: SegmentItem[], currentSegment: SegmentItem): boolean {
  return index === 0 || (segments[index - 1].end < currentSegment.start && segments[index - 1].isEqual);
}

function isNonEmptyEqualLine(line: DiffResult): boolean {
...
export function preprocessDiffs(
  ...
  if (!Array.isArray(diff)) {
    throw new TypeError("Diff input must be an array");
  }
...
export function buildViewFromSegments(
  ...
  if (!Array.isArray(segments) || !Array.isArray(diff)) {
    throw new TypeError("Both segments and diff must be arrays");
  }
