...
function getOriginalIndices(arr: string[], separatorLength: number) {
  const result: number[] = [];
  let index = 0;
...
function filterEmptyParts(arr: InlineDiffResult[]) {
  return arr.filter(item => item.end > item.start);
}

function getInlineDiff(l: string, r: string, options: InlineDiffOptions): [
  InlineDiffResult[],
  InlineDiffResult[],
] {
