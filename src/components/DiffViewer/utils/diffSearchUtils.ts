...
export const performSearch = (term: string, leftView: DiffRowOrCollapsed[]): number[] => {
    if (!term) return [];
...
export const highlightMatches = (term: string, className: string = "json-diff-viewer-theme-custom"): void => {
    if (!term) {
